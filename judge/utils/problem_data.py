...
RUN_MULTI_GRADER_TEMPLATE = """# Auto-generated by ProblemDataCompiler - Polygon run-count support.
# Output cua luot chay i lam input cua luot i+1; checker cham output luot cuoi.
import tempfile

from dmoj.graders.{module} import {base} as _Base
...
class Grader(_Base):
    def grade(self, case):
        ...
        for run in range(RUN_COUNT):
            ...
            self.populate_result(error, result, self._current_proc)
            if result.result_flag:  # RTE/TLE/MLE o bat ky luot nao -> dung
                break
            if run + 1 < RUN_COUNT:
                tmp = tempfile.TemporaryFile()
                tmp.write(result.proc_output or b'')
                tmp.flush()
                tmp.seek(0)
                input_file = tmp
...
class ProblemDataCompiler(object):
    ...
    def make_init(self):
        ...
        def make_grader(init, case):
            ...
            if case.grader == 'standard':
                ...
                self.make_run_multi_grader(init, case, grader_args)
                return
    ...
    def make_run_multi_grader(self, init, case, grader_args):
        """Polygon run-count=N -> sinh custom_judge chay N luot (noi output -> input)."""
        try:
            run_count = int(grader_args.get('run_count', 1))
        except (TypeError, ValueError):
            raise ProblemDataError(_('run_count must be an integer.'))
        if run_count <= 1:
            return
        bases = {
            'standard': ('standard', 'StandardGrader'),
            'signature': ('signature', 'SignatureGrader'),
        }
        if case.grader not in bases:
            raise ProblemDataError(_('run_count > 1 is only supported with standard/signature grader.'))
        module, base = bases[case.grader]
        content = RUN_MULTI_GRADER_TEMPLATE.format(module=module, base=base, run_count=run_count)

        from judge.models import problem_data_storage
        path = '%s/polygon_run_multi.py' % self.problem.code
        if problem_data_storage.exists(path):
            problem_data_storage.delete(path)
        problem_data_storage.save(path, ContentFile(content))
        init['custom_judge'] = 'polygon_run_multi.py'
